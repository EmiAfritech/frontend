        ...
        <Box sx={style}>
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2 }}>
            {Object.entries(params.row).map(([key, value]) => (
              <div key={key}>
                {key}: {key === 'id' ? value : <input type="text" value={value} readOnly />}
              </div>
            ))}
          </Typography>
          ...
          <div className="flex justify-between mt-4">
            <Button onClick={close}>Close</Button>
            <div>
              <Button className="mx-2" variant="contained" color="primary">Edit</Button>
              <Button className="mx-2" variant="contained" color="secondary">Delete</Button>
            </div>
          </div>
        </Box>
      </Modal>
